(function(w, d, u) {
	...
	var str = "<tr>" + "<th>" + '内容名称' + "</th>" + "<th>" + '数量' + "</th>"
			+ "<th>" + '价格' + "</th>" + "</tr>";
	if (products != null) {
		for (var i = 0; i < products.length; i++) {
			str = str + "<tr>" + "<td>" + products[i].title + "</td>" + "<td>"
					+ "<span class=\"lessNum\">" + "-" + "</span>"
					+ "<span class=\"totalNum\" id=\"allNum\">"
					+ products[i].num + "</span>" + "<span id=\"thisId\">"
					+ products[i].id + "</span>" + "<span class=\"moreNum\">"
					+ "+" + "</span>" + "</td>" + "<td>" + products[i].price
					+ "</td>" + "</tr>";
		}
	}
					...
					if (xhr.readyState == 4) {
						...
						if (status >= 200 && status < 300 || status == 304) {
							...
							if (json && json.code == 200) {
								...
								// alert(json.message);
								loading.result(json.message, function() {
									location.href = '/account';
								});
								util.deleteCookie(name);
							}
